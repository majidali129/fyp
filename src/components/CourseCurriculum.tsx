...
const CourseCurriculum = ({ curriculum, isWatching }: CurriculumProps) => {
  return (
    <div className=" border border-gray-100 py-0 flex items-start gap-x-5 rounded-sm">
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="item-1">
          <AccordionTrigger className="[&>div]:relative [&>div>svg]:absolute *:py-3 [&>div]:px-3 [&>div]:ps-7 [&>div>svg]:left-1 [&>div>svg]:top-1/2 [&>div>svg]:-translate-y-1/2 [&[data-state=open]>div>div>div>div>h6]:text-primary-500  ">
            <div className="flex-between w-full">
              ...
              <div className="w-full text-start">
                ...
                {isWatching && (
                  <div className="flex items-center gap-x-0.5 md:hidden">
                    <BiCheckDouble className="w-4 h-4 text-success-500" />
                    <span>25% finish</span>
                    <span className="text-gray-500">(1/4)</span>
                  </div>
